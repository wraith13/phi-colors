export module phiColors
{
    ...
    export const rgbForStyle = (expression: Rgb) =>
        "#"
            ...
            +toHex(expression.g)
            +toHex(expression.b);
    export const rgbForCssColor = (colorSpace: string, expression: Rgb, digits: number = 5) =>
        `color(${colorSpace} ${expression.r.toFixed(digits)} ${expression.g.toFixed(digits)} ${expression.b.toFixed(digits)})`;
    export const rgbaForCssColor = (colorSpace: string, expression: Rgba, digits: number = 5) =>
